...
public final class PasswordFile {
    ...
    // encryption.
    private static void writeToFile(String fileName, Cipher cipher, ArrayList<PasswordEntry> data) throws IOException {
        ...
        ObjectOutputStream output = new ObjectOutputStream(new CipherOutputStream(new BufferedOutputStream(new FileOutputStream(fileName)), cipher));
    ...
    public static void encryptStore(String file, byte[] key, ArrayList<PasswordEntry> data) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, IOException {
        ...
        writeToFile(file, cipher, data);
    }

    // This is an alias for the other encryptStore() method that takes a String for the key instead
    // of an array of bytes.
    public static void encryptStore(String file, String key, ArrayList<PasswordEntry> data) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, IOException {
        encryptStore(file, key.getBytes(), data);
    }
    ...
    private static ArrayList<PasswordEntry> readFromFile(String fileName, Cipher cipher) throws IOException, ClassNotFoundException {
        ...
        ObjectInputStream input = new ObjectInputStream(new CipherInputStream(new BufferedInputStream(new FileInputStream(fileName)), cipher));
    ...
    public static ArrayList<PasswordEntry> decryptStore(String file, byte[] key) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, IOException, ClassNotFoundException {
        // Create the necessary Cipher object based on the key.
        SecretKeySpec sks = new SecretKeySpec(key, ALGORITHMTYPE);
        Cipher cipher = Cipher.getInstance(TRANSFORMATION);
        ...
        return readFromFile(file, cipher);
    }

    // This is an alias for the other decryptStore() method that takes a String for the key instead
    // of an array of bytes.
    public static ArrayList<PasswordEntry> decryptStore(String file, String key) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, IOException, ClassNotFoundException {
        return decryptStore(file, key.getBytes());
    }
}
